...
void cmd_hist (int paramN, char* param[], List* hist)
{
    if (!paramN){
        print_n_elements(null, hist);
    }

    if (paramN == 2 && !strcmp(param[0], "-N") && is_number(param[1])){
        print_n_elements(atoi(param[1]), hist);
    }

    if (paramN == 1 && !strcmp(param[0], "-c")){
        clear_list(hist);
    }
...
void cmd_repeat (int paramN, char* params[])
{
    if (paramN == 1 && is_number(params[0])){
        char* command = get_element(atoi(params[0]), &history);
